...
void scanner::initScannerIndexPage()
{
   ...
   timelabel->setFont(qf);
    ...
    numlabel->setText(tr("已扫描：")+QString::number(0)+tr("页"));
    connect(timer2, &QTimer::timeout,
            this,[=]{
         numlabel->setText(tr("已扫描：")+QString::number(readfloadimage())+tr("页")+tr("    总数:")+QString::number(readfloadimage2())+tr("页"));
    });
    timer2->start(1000);
...
//    });
    ...
    connect(imagelWidget,&MyWidget::clicked,this,[=]{
     ...
     //设置窗
      ...
      title3=new Mylabel();
      QString tempname3=tr("设置");
       title3->setText(tempname3);//Settings
        title3->setFont(qf2);
       title3->move(78,140);//70
...
void scanner::scannerwidgeclick()
{
    ...
    //    system("pwd > /mnt/scannertest.txt");
        QProcess *process=new QProcess();
         QString workspace=QCoreApplication::applicationDirPath()+readJsonfloadname("scannerworkspace");
         QString shcode="sh "+readJsonfloadname("scannershellname");
         process->setWorkingDirectory(workspace);
         process->start(shcode);

         if(process->waitForFinished()){
             process->close();
             if(sunimagewidget!=nullptr){
                 delete(sunimagewidget);
                 sunimagewidget=nullptr;
                 sunimagewidget=new imagewidget();
                 falg=true;
...
//  qDebug()<<"设置框点击事件"<<endl;
  QProcess *process=new QProcess();
   ...
   process->setWorkingDirectory(workspace);
   process->start("sh settshellcode.sh");
   if(process->waitForFinished()){
       process->close();
   }
...
void scanner::checkscannfinsh(){
         ...
         if(!qfile.exists()){
             QMessageBox *msgBox=new QMessageBox(QMessageBox::Information, tr("扫描成功"), "", QMessageBox::Ok|QMessageBox::Cancel, this);
                msgBox->setText("扫描"+QString::number(readfloadimage())+"页");
