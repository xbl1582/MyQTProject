...
//    //一个图片预览列表
   QStringList buttonstring={tr("上一页"),tr("下一页"),tr("水印"),tr("黑边"),tr("全选"),tr("取消"),tr("导出"),tr("删除"),tr("清空"),tr("刷新"),tr("返回")};
   //
  ...
  QCoreApplication::applicationDirPath()+"/icon/huanyuan",
   ...
   QString choosetext="当前页:"+QString::number(page)+QString("/")+QString::number(allpage)+" "
           +"选中:"+QString::number(choose)+"/"+QString::number(allimage)+"";
...
//
                    ...
                    QString choosetext=tr("当前页:")+QString::number(page+1)+QString("/")+QString::number(pagesize)+" "
                            +tr("选中:")+QString::number(choose)+"/"+QString::number(allimage)+"";
...
{
    if(watercontent==""){
        QMessageBox msgBox(QMessageBox::Warning, tr("水印内容不能为空"), tr("水印内容不能为空"), QMessageBox::Ok, this);
        msgBox.setWindowFlag(Qt::WindowStaysOnTopHint);
        int result = msgBox.exec();
    }else{
     //保存水印位置
...
void FolderWidget::quanxuan()
{
    ...
    buttons.at(3)->setIcon(QIcon(QCoreApplication::applicationDirPath()+"/icon/yebianju.png"));
     ...
     QString choosetext=tr("当前页:")+QString::number(page+1)+QString("/")+QString::number(pagesize)+" "
             +tr("选中:")+QString::number(choose_list.size())+"/"+QString::number(allimage)+" ";
     label->setText(choosetext);
     changeChooseColor();
...
void FolderWidget::quxiaoxuanze()
{
    if(choose_list.size()>0){
       ...
       buttons.at(6)->setIcon(QIcon(QCoreApplication::applicationDirPath()+"/icon/daochu2.png"));
        ...
        buttons.at(6)->setIcon(QIcon(QCoreApplication::applicationDirPath()+"/icon/daochu2.png"));
          buttons.at(7)->setIcon(QIcon(QCoreApplication::applicationDirPath()+"/icon/deleter2.png"));
//        QMessageBox msgBox(QMessageBox::Warning, tr("请先选择目录"), tr("请先选择目录"), QMessageBox::Ok, this);
//        msgBox.setWindowFlag(Qt::WindowStaysOnTopHint);
//        int result = msgBox.exec();
    }
}
...
void FolderWidget::deleteimage2()
{
    ...
    QFont qf;
    qf.setPixelSize(20);
    mylabel->setText(tr("确定删除")+QString::number(string_list.size())+tr("张图片?"));
    mylabel->setFont(qf);
    QFormLayout form(&dialog);
     QString value1 = QString("");
     form.addRow(value1,mylabel);
